...
5. 若需要改写正文，修改源章节，保留旧结论的历史记录并说明变化原因。同步更新 sources、社区雷达、CHANGELOG 和状态；无变化时只更新必要的检索运行记录，不制造“进展”。
6. 运行 `python3 scripts/build_survey.py` 和 `python3 scripts/validate.py`。正文变化后运行 PDF 构建并检查渲染；PDF构建失败时标明PDF落后版本，不能把旧PDF冒充最新。
...
源章节和证据是事实来源；SURVEY.zh-CN.md、REFERENCES.md、references.bib、data/papers.json 是生成产物。PDF是阅读版，公式精确表达仍以Markdown为准。文献库不要求一次收齐所有作者，但缺失字段必须保留真实状态；任何后续补全应核对主源。
